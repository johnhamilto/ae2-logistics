...
public class TracePanelScreen extends AEBaseScreen<TracePanelMenu> {
    ...
    protected void init() {
        ...
        for (int i = 0; i < removeButtons.length; i++) {
            ...
            removeButtons[i] = new AE2Button(leftPos + imageWidth - 60, topPos + ROW_TOP + row * ROW_HEIGHT,
                    50, 14, Component.literal("Remove"), b -> {
                        var bound = channels();
                        if (row < bound.size()) {
                            ClientPacketDistributor.sendToServer(new TracePanelActionPayload(
        ...
        clearButton = new AE2Button(leftPos + 10, topPos + imageHeight - 26, 70, 16,
                Component.literal("Clear all"), b -> ClientPacketDistributor.sendToServer(
                        new TracePanelActionPayload(menu.pos, Optional.empty())));
        addRenderableWidget(clearButton);
    }

    private List<Identifier> channels() {
    ...
    public void drawFG(GuiGraphicsExtractor guiGraphics, int offsetX, int offsetY, int mouseX, int mouseY) {
        var bound = channels();
        if (bound.isEmpty()) {
            guiGraphics.text(font, "No traces bound.", 10, ROW_TOP + 3, Palette.HINT, false);
            guiGraphics.text(font, "Click the panel with a bound Signal Card.",
                    10, ROW_TOP + 15, Palette.HINT, false);
            return;
        }
        for (int i = 0; i < bound.size() && i < removeButtons.length; i++) {
            guiGraphics.text(font, bound.get(i).toString(), 10, ROW_TOP + i * ROW_HEIGHT + 3,
                    Palette.VALUE, false);
        }
