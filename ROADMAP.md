...
Status as of v0.44.0 (2026-08-13). DESIGN.md holds the full rationale; this file tracks
what exists, what is queued, and what is known debt. The gametest suite (142 tests, run
...
| F13 | V2: data rules ride the query language - a data: term (path into the item's serialized component tree, glob only at the located node; the patch tree caches per immutable key; NOT flat-string matching - a gametest pins that "mending" in a custom name never trips an enchantment rule while the whole-tree form does see it) plus a Query Card that binds a NAMED query like a Signal Card binds a channel and turns a gated-bus-family partition into live query membership (resolved against the Query Terminal library per insert; precedence Query > Variant > stock) - so NBT-pattern routing composes with tags/mod/craftable/signals for free; QueryContext gained registries (from the grid pivot) for component serialization; V3 (click-to-filter tree editor compiling to the same path rules) designed in DESIGN F13 and deferred to an in-game GUI loop | 0.44.0 |
